...
void Foam::phaseCompressibleSystem::setModels(const dictionary& dict)
{
    if (Foam::max(this->thermo().mu()).value() > 0)
    {
        ...
        (
            ...
            (
                rho_,
                U_, //(include stress tensor tau here? (UR))
                rhoPhi_,
                this->thermo()
            )
        );
...
)
{
    ...
    )
    {
        ...
        // sr added by Usman Rana
       sr_ = sqrt(2.0)*mag(symm(fvc::grad(U_)));
       sr_.correctBoundaryConditions();


       
       // tau added by Usman Rana (needed?)
      tau_.correctBoundaryConditions();

          // (right location for tau_? (Usman Rana))
      
         // Velocity gradient tensor added by Usman Rana
         volTensorField L = fvc::grad(U_);

        // Convected derivate term added by Usman Rana
        volTensorField C = tau_ & L;
        
        // Twice the rate of deformation tensor added by Usman Rana
        volSymmTensorField twoD = twoSymm(L);
        
        // random values for eta and lambda added by Usman Rana!
        
       
        // stress transport equation added by Usman Rana
       ...
       tauEqn.solve();
        ...
        if (turbulence_.valid())
        {
            ...
            rhoE_ = rho_*(e() + 0.5*magSqr(U_)); // Includes change to total energy from viscous term in momentum equation 
                                                 // no other changes needed in case of visocelastic flows (comment UR)
