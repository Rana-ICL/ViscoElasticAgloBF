...
Foam::lookupTable2D::reverseLookupY(const scalar& fin, const scalar& x) const
{
  ...
  Info << "Hello f " << f << " " << fin << " " << x << endl;
    scalar fx;
  Info << "Hello fx " << f << endl;
    label i;
  Info << "Hello i " << modXFunc_(x) << " " << xMin_ << " " << dx_ << " " << nx_ << " " << i << endl;
    findIndex(modXFunc_(x), xMin_, dx_, nx_, i, fx);
  Info << "Hello " << f << " "  << i << endl;
    label j = bound(f, i, yMin_, dy_, ny_, true);
  Info << "Hello findIndex " << i << " " << j << " " << data_[i][j] << endl;
//  Info << "Hello findIndex " << i << " " << j << " " << data_[i] << endl;

    const scalar& mm(data_[i][j]);
  Info << "Hello mm " << mm << endl;
    const scalar& pm(data_[i+1][j]);
  Info << "Hello pm " << pm << endl;
    const scalar& mp(data_[i][j+1]);
  Info << "Hello mp " << mp << endl;
    const scalar& pp(data_[i+1][j+1]);
  Info << "Hello pp " << pp << endl;
